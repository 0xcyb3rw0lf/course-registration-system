...
if (isset($_POST["add-program"])) {
    ...
    } else if (!preg_match("/[A-Za-z]+/", $pname)) { // if the name contains numbers
        $feedbackMsg = "<span class='failed-feedback'>Program Name Should Contains Only Letters!</span>";
    } else {
        try {
            if (addProgram($collegeId, $pname)) {
                $feedbackMsg = "<span class='success-feedback'>Program is Added Successfully!</span>";
            } else {
                $feedbackMsg = "<span class='failed-feedback'>Error Adding Program!</span>";
            }
        } catch (Exception $exception) { // send error if program already exists
            $feedbackMsg = "<span class='failed-feedback'>Program Already Exists, Please Choose a New Program Name!</span>";
        }
    }
}
?>
...
<body>
    ...
    <main class="payment-main" style="background-color: white; background-image: none; text-align: left;">
        <h1 class="catalogue-header" style="color: #4056A1;">Add Program</h1>
        ...
        <form method="post" class="form" style="margin-left: 2.75em;">
            <div class="attendance-flex catalogue-main">
                ...
                <div class="attendance-inner-flex" style="margin-left: 2.5em;">
                    ...
                    <input type="text" class="selecter" name="program-name" id="program-name">
                    </select>
                    <br><br>
                </div>
            ...
            <input onclick="return confirm('Are you sure you want to add a program?')" type="submit" class="butn primary-butn sign-butn no-margin-left margin-top small" name="add-program" id="add-program" value="Add a Program">
            <br><br>
            <?php
